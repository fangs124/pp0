...
impl ChessMove {
    ...
    pub const fn move_type(&self) -> MoveType {
        let (piece, castling): (PieceType, Castling) = match ((self.data.get() & 0b11_000000_000000u16) as usize) >> 12 {
    ...
    pub fn add_pawn_moves(s: Square, ts: Bitboard, moves: &mut MoveList) {
        ...
        while ts_promotion.is_not_zero() {
            ...
            moves.extend_from_slice(&ChessMove::promotions(s, t));
    ...
    pub const fn new(s: Square, t: Square, m: MoveType) -> Self {
        ...
        let mut data: u16 = ((s.as_usize() & 0b111111) | ((t.as_usize() << 6) & 0b111111_000000)) as u16;
