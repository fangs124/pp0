...
use crate::{constdata, init, BitBoard, Side};

//todo: fix this.. tried making these arrays static, but they can no longer be called in a const fn.
pub(crate) const W_PAWN_ATTACKS: [BitBoard; 64] = init::pawn_attack(Side::White);
pub(crate) const B_PAWN_ATTACKS: [BitBoard; 64] = init::pawn_attack(Side::Black);
pub(crate) const KNIGHT_ATTACKS: [BitBoard; 64] = init::knight_attack();
pub(crate) const KING_ATTACKS: [BitBoard; 64] = init::king_attack();
pub(crate) const BISHOP_MBB_MASK: [BitBoard; 64] = init::bishop_mbb_mask();
pub(crate) const ROOK_MBB_MASK: [BitBoard; 64] = init::rook_mbb_mask();
pub(crate) const BISHOP_ATTACKS_MBB: [[BitBoard; 1 << 9]; 64] = constdata::BISHOP;
pub(crate) const ROOK_ATTACKS_MBB: [[BitBoard; 1 << 12]; 64] = constdata::ROOK;
